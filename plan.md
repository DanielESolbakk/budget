...
4. Phase 2: Implement the import pipeline. Build import flows for CSV, digital text PDFs, and manual transaction entry. Add file validation, parser adapters per source, duplicate detection, import preview, and failure reporting. Keep scanned-image OCR out of the first milestone unless sample PDFs prove it is required; design the importer so OCR can be added as a parser adapter later. This depends on step 3.
